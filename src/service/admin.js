...
<body><main>
  <a href="/">← 首页</a><div class="eyebrow" style="margin-top:36px">METING / PRIVATE CONTROL</div><h1>Cookie 管理</h1><p>通过 <code>METING_TOKEN</code> 登录。Cookie 仅写入 KV，读取列表不会返回其内容。</p>
  ...
  <script>
    const tokenInput = document.getElementById('token'); const login = document.getElementById('login'); const manager = document.getElementById('manager'); const message = document.getElementById('message'); const managerMessage = document.getElementById('manager-message'); const cookies = document.getElementById('cookies');
    const request = (path, options = {}) => fetch(path, { ...options, headers: { ...options.headers, Authorization: 'Bearer ' + sessionStorage.getItem('meting-admin-token') } });
    async function load() { const response = await request('/admin/cookies'); if (!response.ok) throw new Error((await response.text()) || '无法读取 Cookie 列表'); const data = await response.json(); cookies.innerHTML = data.cookies.length ? data.cookies.map(cookie => '<div class="row"><div><div>' + cookie.id + '</div><div class="platform">' + cookie.platform + '</div></div><button class="danger" data-platform="' + cookie.platform + '" data-id="' + cookie.id + '">删除</button></div>').join('') : '<p>KV 中尚未添加 Cookie。</p>'; cookies.querySelectorAll('button').forEach(button => button.onclick = async () => { if (!confirm('确认删除此 Cookie？')) return; const response = await request('/admin/cookies/' + button.dataset.platform + '/' + button.dataset.id, { method: 'DELETE' }); if (!response.ok) { managerMessage.textContent = '删除失败'; return } await load() }); }
    async function enter() { sessionStorage.setItem('meting-admin-token', tokenInput.value); try { await load(); login.classList.add('hidden'); manager.classList.remove('hidden') } catch (error) { sessionStorage.removeItem('meting-admin-token'); message.textContent = 'Token 无效或 KV 未绑定' } }
    document.getElementById('login-button').onclick = enter; tokenInput.addEventListener('keydown', event => { if (event.key === 'Enter') enter() }); document.getElementById('add-button').onclick = async () => { const cookie = document.getElementById('cookie').value; const platform = document.getElementById('platform').value; managerMessage.textContent = ''; const response = await request('/admin/cookies', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ platform, cookie }) }); if (!response.ok) { managerMessage.textContent = '添加失败，请检查 Cookie'; return } document.getElementById('cookie').value = ''; await load() };
  </script>
</main></body></html>`)
